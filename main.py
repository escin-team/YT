...
app = FastAPI(
    ...
    version="1.0.0",
    description="REST API service for YouTube search, playlist metadata extraction, and audio download",
    docs_url="/api/docs",
...
# Configure static file mount (for downloaded audio files)
config = get_settings()
download_dir = Path(config.download_dir)
if download_dir.exists():
    app.mount("/downloads", StaticFiles(directory=str(download_dir)), name="downloads")
    logger.info(f"Static files mounted: /downloads -> {download_dir}")
else:
